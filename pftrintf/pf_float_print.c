...
static char 			*pf_itoafloat(double i, int num)
{
	char 		*str;
...
static void				pf_union_and_write(t_type *buff, char *whole, char *rest)
{
	char	*leak;
	char 	*accurrest;
	char 	*str1;
	...
	if ((buff->accur->number == 0 && buff->flag->hesh == 1) || buff->accur->number > 0)
	{
		leak = whole;
...
void					pf_print_float(t_type *buff, double num)
{
	...
	pf_union_and_write(buff, flist.whole, flist.rest);
//	printf("%s, %s\n", flist.whole, flist.rest);
}